                 ...
                 PyObject *kwnames)
{
    ...
    if (nargs > 1 && nargs <= 10) {
        if (PyErr_WarnEx(PyExc_DeprecationWarning,
                ...
                "'cached_statements', 'uri', 'flags', and 'vfs' will become keyword-only "
...
#define ADD_INT(ival)                                           \
    ...
    ADD_INT(SQLITE_OPEN_CREATE);
    ADD_INT(SQLITE_OPEN_FULLMUTEX);
    ADD_INT(SQLITE_OPEN_MEMORY);
    ADD_INT(SQLITE_OPEN_NOMUTEX);
    ADD_INT(SQLITE_OPEN_PRIVATECACHE);
    ADD_INT(SQLITE_OPEN_READONLY);
    ADD_INT(SQLITE_OPEN_SHAREDCACHE);
    ADD_INT(SQLITE_OPEN_READWRITE);
    ADD_INT(SQLITE_OPEN_URI);
...
module_exec(PyObject *module)
{
    ...
    if (PyModule_AddStringConstant(module, "version", PYSQLITE_VERSION)) {
        goto error;
    }
    if (PyModule_AddObjectRef(module, "OptimizedUnicode",
