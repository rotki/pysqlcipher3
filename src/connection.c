...
_sqlite3.Connection.__init__ as pysqlite_connection_init
    ...
    * [from 3.15]
    ...
    uri: bool = False
    flags: int(c_default='SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE') = SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE
    vfs: str(accept={str, NoneType}) = None
    *
...
pysqlite_connection_init_impl(pysqlite_Connection *self, PyObject *database,
                              ...
                              int cache_size, int uri, int flags,
                              const char *vfs,
                              enum autocommit_mode autocommit)
/*[clinic end generated code: output=37712005d141dfd7 input=55d49d2c322b30fd]*/
{
...
connection_traverse(PyObject *op, visitproc visit, void *arg)
{
    ...
    VISIT_CALLBACK_CONTEXT(self->busy_ctx);
...
connection_clear(PyObject *op)
{
    ...
    clear_callback_context(self->busy_ctx);
    return 0;
}
...
free_callback_contexts(pysqlite_Connection *self)
{
    ...
    set_callback_context(&self->busy_ctx, NULL);
}
...
static PyObject *
pysqlite_connection_open_blob(pysqlite_Connection *self, PyObject *args,
                              PyObject *kwargs)
{
    static char *keywords[] = {
        "table", "column", "row", "readonly", "dbname", NULL
    };
    const char *table;
    const char *column;
    sqlite3_int64 row;
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "ssL|ps:open_blob", keywords,
            &table, &column, &row, &readonly, &dbname)) {
        return NULL;
    }

    return blobopen_impl(self, table, column, row, readonly, dbname);
}
...
static int
busy_callback(void *ctx_vp, int count)
{
    PyGILState_STATE gilstate = PyGILState_Ensure();
    callback_context *ctx = (callback_context *)ctx_vp;
    incref_callback_context(ctx);

    PyObject *arg = PyLong_FromLong(count);
    PyObject *ret = arg != NULL ? PyObject_CallOneArg(ctx->callable, arg) : NULL;
    Py_XDECREF(arg);
...
static PyObject *
pysqlite_connection_set_busy_handler(pysqlite_Connection *self,
                                     PyTypeObject *cls,
                                     PyObject *const *args,
                                     Py_ssize_t nargs,
                                     PyObject *kwnames)
{
    static const char * const keywords[] = {"busy_handler", NULL};
    static _PyArg_Parser parser = {
        .keywords = keywords,
        .fname = "set_busy_handler",
    };
    PyObject *argsbuf[1];

    args = _PyArg_UnpackKeywords(
        args, nargs, NULL, kwnames, &parser, 1, 1, 0, 0, argsbuf);
    if (args == NULL) {
        return NULL;
    }
    ...
    if (callable == Py_None) {
        ...
        if (!PyCallable_Check(callable)) {
            PyErr_SetString(PyExc_TypeError, "busy_handler must be callable");
            return NULL;
        }
        callback_context *ctx = create_callback_context(cls, callable);
        if (ctx == NULL) {
        ...
        rc = sqlite3_busy_handler(self->db, busy_callback, ctx);
        set_callback_context(&self->busy_ctx, ctx);
    }

    if (rc != SQLITE_OK) {
        PyErr_SetString(self->OperationalError, "Error setting busy handler");
        set_callback_context(&self->busy_ctx, NULL);
        return NULL;
    }
...
static PyObject *
pysqlite_connection_set_busy_timeout(pysqlite_Connection *self, PyObject *args,
                                     PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "d:set_busy_timeout", keywords, &timeout)) {
        return NULL;
    }
    ...
    int rc = sqlite3_busy_timeout(self->db, (int)(timeout * 1000));
    if (rc != SQLITE_OK) {
        PyErr_SetString(self->OperationalError, "Error setting busy timeout");
        return NULL;
    }
...
static PyObject *
pysqlite_connection_key(pysqlite_Connection *self, PyObject *args)
{
    ...
    rc = sqlite3_key(self->db, key_buffer.buf, (int)key_buffer.len);
    PyBuffer_Release(&key_buffer);

    if (rc != SQLITE_OK) {
        PyErr_SetString(self->OperationalError, sqlite3_errstr(rc));
        return NULL;
    }
    Py_RETURN_NONE;
}
...
static PyObject *
pysqlite_connection_rekey(pysqlite_Connection *self, PyObject *args)
{
    Py_buffer key_buffer;
    int rc;
    ...
    rc = sqlite3_rekey(self->db, key_buffer.buf, (int)key_buffer.len);
    PyBuffer_Release(&key_buffer);
...
static PyMethodDef connection_methods[] = {
    ...
    BLOBOPEN_METHODDEF
    {"open_blob", (PyCFunction)(void(*)(void))pysqlite_connection_open_blob,
     METH_VARARGS | METH_KEYWORDS,
     PyDoc_STR("Open and return a BLOB object.")},
    SETCONFIG_METHODDEF
    GETCONFIG_METHODDEF
    {"set_busy_handler", _PyCFunction_CAST(pysqlite_connection_set_busy_handler),
     METH_METHOD | METH_FASTCALL | METH_KEYWORDS,
     PyDoc_STR("Sets busy handler. Non-standard.")},
    {"set_busy_timeout", (PyCFunction)(void(*)(void))pysqlite_connection_set_busy_timeout,
     METH_VARARGS | METH_KEYWORDS,
     PyDoc_STR("Sets busy timeout. Non-standard.")},
#ifdef HAVE_ENCRYPTION
    {"set_key", (PyCFunction)pysqlite_connection_key, METH_VARARGS,
     PyDoc_STR("Set encryption key for database. Non-standard.")},
    {"reset_key", (PyCFunction)pysqlite_connection_rekey, METH_VARARGS,
     PyDoc_STR("Set encryption key for database. Non-standard.")},
#endif
    {NULL, NULL}
};
