...
pysqlite_connection_init(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    static struct {
        ...
        .ob_hash = -1,
        .ob_item = { &_Py_ID(database), &_Py_ID(timeout), &_Py_ID(detect_types), &_Py_ID(isolation_level), &_Py_ID(check_same_thread), &_Py_ID(factory), &_Py_ID(cached_statements), &_Py_ID(uri), &_Py_ID(flags), &_Py_ID(vfs), &_Py_ID(autocommit), },
    };
    #undef NUM_KEYWORDS
    ...
    static const char * const _keywords[] = {"database", "timeout", "detect_types", "isolation_level", "check_same_thread", "factory", "cached_statements", "uri", "flags", "vfs", "autocommit", NULL};
    ...
    if (nargs > 1 && nargs <= 10) {
        if (PyErr_WarnEx(PyExc_DeprecationWarning,
                ...
                "'cached_statements', 'uri', 'flags' and 'vfs' will become "
                "keyword-only parameters in Python 3.15.", 1))
        {
            goto exit;
        }
    }
    fastargs = _PyArg_UnpackKeywords(_PyTuple_CAST(args)->ob_item, nargs, kwargs, NULL, &_parser,
            /*minpos*/ 1, /*maxpos*/ 10, /*minkw*/ 0, /*varpos*/ 0, argsbuf);
    if (!fastargs) {
        goto exit;
    }
    database = fastargs[0];
    ...
    if (fastargs[9]) {
        if (fastargs[9] == Py_None) {
            vfs = NULL;
        }
        else if (PyUnicode_Check(fastargs[9])) {
            Py_ssize_t vfs_length;
            vfs = PyUnicode_AsUTF8AndSize(fastargs[9], &vfs_length);
            if (vfs == NULL) {
                goto exit;
            }
            if (strlen(vfs) != (size_t)vfs_length) {
                PyErr_SetString(PyExc_ValueError, "embedded null character");
                goto exit;
            }
        }
        else {
            _PyArg_BadArgument("Connection", "argument 'vfs'", "str or None", fastargs[9]);
            goto exit;
        }
        if (!--noptargs) {
            goto skip_optional_pos;
...
skip_optional_pos:
    ...
    if (!autocommit_converter(fastargs[10], &autocommit)) {
        goto exit;
    }
skip_optional_kwonly:
    return_value = pysqlite_connection_init_impl((pysqlite_Connection *)self, database, timeout, detect_types, isolation_level, check_same_thread, factory, cache_size, uri, flags, vfs, autocommit);
...
/*[clinic end generated code: output=d825d37bb25de48d input=a9049054013a1b77]*/
